    ...
    public class UserController : Controller
    {
            ...
            string paymentMethod)
        {
            ...
            if (!string.Equals(paymentMethod, Order.PaymentMethodOnlineDeposit, StringComparison.Ordinal)
                && !string.Equals(paymentMethod, Order.PaymentMethodOnlineFull, StringComparison.Ordinal)
                && !string.Equals(paymentMethod, Order.PaymentMethodCashDeposit, StringComparison.Ordinal))
            {
                ModelState.AddModelError(nameof(paymentMethod), "Vui lòng chọn hình thức thanh toán.");
            }
            ...
            if (string.Equals(paymentMethod, Order.PaymentMethodOnlineFull, StringComparison.Ordinal))
            {
                TempData["SuccessMessage"] = $"Đã tạo đơn #{order.OrderNumber}. Vui lòng thanh toán toàn bộ {depositAmount:N0} ₫ trong 1 giờ 30 phút để đơn chuyển sang chờ xác nhận.";
            }
            else
            {
        ...
        private async Task CancelExpiredDepositOrdersAsync()
        {
            ...
            {
                order.Status = Order.StatusCancelled;
                order.CancelReason = "Đơn hàng tự hủy vì khách chưa hoàn tất thanh toán trong 1 giờ 30 phút.";
