...
// Lỗi dịch vụ nền không được làm dừng toàn bộ website.
...
builder.Services.Configure<OnlinePaymentWebhookOptions>(builder.Configuration.GetSection("OnlinePaymentWebhook"));
...
// Bảo đảm đầy đủ vai trò và tài khoản quản trị mặc định.
using (var roleScope = app.Services.CreateScope())
{
    try
    {
        ...
        const string adminEmail = "admin@goldsys.com";
        ...
        if (adminUser == null)
        {
            ...
            else
            {
                ...
                app.Logger.LogWarning("Không thể tạo tài khoản Admin mặc định: {Errors}", errors);
            }
        }
    ...
    {
        // Cho phép website khởi động khi SQL Server tạm thời mất kết nối.
